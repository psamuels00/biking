...
def format_input_record(rec, empty="", limit_precision=True):
    return dict(
        ...
        time=format_numeric(rec["moving_time"], empty, limit_precision, 1),
...
def format_metrics_record(rec, empty="", limit_precision=True):
    return dict(
        ...
        work=format_numeric(rec["work"], empty, limit_precision),
        avg_work=format_numeric(rec["avg_work"], empty, limit_precision),
